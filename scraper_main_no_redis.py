...
Just logs URLs found - no database communication for better performance
"""
import time
import logging
import signal
...
from scraper_class_no_redis import scrape_task_no_redis
...
# ============================================================================
# SIGNAL HANDLING FOR MID-TASK STOPPING
# ============================================================================

def signal_handler(sig, frame):
    """Handle shutdown signals gracefully - stops browser and ends process"""
    logger.info("")
    logger.info("="*60)
    logger.info("🛑 SHUTDOWN SIGNAL RECEIVED")
    logger.info("="*60)
    logger.info("Stopping current tasks...")
    logger.info("Browsers will be closed gracefully...")
    shutdown_event.set()
...
def execute_task(task: dict):
    ...
    try:
        # Execute task without Redis - just logging
        stats = scrape_task_no_redis(
            task=task,
            max_pages=MAX_PAGES_PER_TASK,
            shutdown_event=shutdown_event  # Pass shutdown event for mid-task stopping
        )
        ...
        with stats_lock:
            ...
            if stats['status'] == 'success':
                ...
                logger.info(f"[{task['task_id']}] Task stopped by shutdown signal")
...
def main():
    """Main execution flow WITHOUT Redis - optimized for performance"""

    print(f"\n{'='*60}")
    print(f" {SITE_NAME.upper()} SCRAPER - OPTIMIZED (NO REDIS)")
    print(f"{'='*60}")
    ...
    print(f" Mode: LOGGING ONLY (No Database)")
    print(f"{'='*60}\n")
    ...
    print(f" URLs Found: {global_stats['total_urls']:,}")
    print(f"{'='*60}\n")
