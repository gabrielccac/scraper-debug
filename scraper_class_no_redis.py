...
# -*- coding: utf-8 -*-
"""
OLX Scraper - Optimized Version WITHOUT Redis
Just logs URLs - no database communication for better performance
"""
import time
import re
import os
...
class OlxScraperNoRedis:
    """
    Optimized scraper for olx.com.br - NO Redis, just logging.
    ...
    - URL logging only (no database storage)
    - Graceful mid-task stopping support
    """
    ...
    def __init__(self):
        """Initialize scraper without Redis (logging only mode)."""
        self.sb = None
        self.debug_dir = os.getenv('FAILED_SCREENSHOTS_DIR', 'debug_screenshots')
        os.makedirs(self.debug_dir, exist_ok=True)
        logger.debug("OlxScraperNoRedis instance created")
    ...
    # ========================================================================
    # LOGGING (replaces Redis storage)
    # ========================================================================

    def log_urls_batch(self, url_price_pairs: list, page_num: int, task_id: str):
        """
        Log URLs to console instead of storing to Redis.
        ...
        logger.info(f"")
        logger.info(f"{'='*80}")
        logger.info(f"📄 PAGE {page_num} - {len(url_price_pairs)} URLs Found - [{task_id}]")
        logger.info(f"{'='*80}")

        for idx, (url, price) in enumerate(url_price_pairs, 1):
            price_str = f"R$ {price:,}" if price else "No price"
            logger.info(f"{idx:3d}. {price_str:>15s} | {url}")
...
# ============================================================================
# PRODUCTION WORKFLOW - Task-Based Scraping WITHOUT Redis
# ============================================================================

def scrape_task_no_redis(task: dict, max_pages: int = 100, shutdown_event: threading.Event = None):
    """
    Optimized workflow: scrape task WITHOUT Redis - just logging URLs.
    ...
    """
    logger.info("="*60)
    logger.info("STARTING SCRAPE TASK (NO REDIS - LOGGING ONLY)")
    ...
    for attempt in range(MAX_RETRIES):
        ...
        # Check for shutdown signal before starting attempt
        if shutdown_event and shutdown_event.is_set():
            logger.info("🛑 Shutdown signal detected before attempt, stopping task")
            total_stats['status'] = 'stopped'
            return total_stats
        ...
        try:
            ...
            # Initialize scraper (no Redis)
            scraper = OlxScraperNoRedis()
            ...
            # Log URLs (instead of storing to Redis)
            scraper.log_urls_batch(page_data, page_num, task['task_id'])
            ...
            while current_page < pagination_limit:
                # Check for shutdown signal during pagination
                if shutdown_event and shutdown_event.is_set():
                    logger.info("")
                    logger.info("="*60)
                    logger.info("🛑 SHUTDOWN SIGNAL RECEIVED DURING PAGINATION")
                    logger.info("="*60)
                    logger.info("Stopping browser and ending process...")
                    total_stats['status'] = 'stopped'
                    return total_stats
            ...
            logger.info(f"📊 Total URLs logged: {total_stats['urls_found']:,}")
            ...
            # Always close browser after each attempt (CRITICAL for cleanup)
            if scraper:
